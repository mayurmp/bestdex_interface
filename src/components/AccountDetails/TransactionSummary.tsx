import { Fraction, TradeType } from '@tech-alchemy/best-dex/sdk-core'
...
function WrapSummary({ info: { currencyAmountRaw, unwrapped } }: { info: WrapTransactionInfo }) {
  ...
  const getNativeWrappedName = (chainId: any) => {
    switch (chainId) {
      case 1:
      case '1':
      case 5:
  ...
  if (unwrapped) {
    return (
      <Trans>
        ...
        <FormattedCurrencyAmount
          ...
          symbol={getNativeWrappedName(chainId)}
          decimals={18}
          sigFigs={6}
        />{' '}
        to
    ...
    return (
      <Trans>
        ...
        <FormattedCurrencyAmount
          rawAmount={currencyAmountRaw}
          symbol={getNativeTokenName(chainId)}
          decimals={18}
          sigFigs={6}
        />{' '}
        to {getNativeWrappedName(chainId)}
      </Trans>
    )
  }
}
