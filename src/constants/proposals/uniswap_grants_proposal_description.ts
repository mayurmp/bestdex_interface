export const SWAP_GRANTS_PROPOSAL_DESCRIPTION = `#`
// `# swap Grants Program v0.1

// *co-authored with [Ken Ng](https://twitter.com/nkennethk?lang=en)*

// ## Summary:

// **This post outlines a framework for funding swap ecosystem development with grants from the[ UNI Community Treasury](https://swap.org/blog/uni/).** The program starts small—sponsoring hackathons, [for example](https://gov.swap.org/c/proposal-discussion/5)—but could grow in significance over time (with renewals approved by governance) to fund core protocol development. Grants administration is a subjective process that cannot be easily automated, and thus we propose a nimble committee of 6 members —1 lead and 5 reviewers—to deliver an efficient, predictable process to applicants, such that funding can be administered without having to put each application to a vote. We propose the program start with an initial cap of $750K per quarter and a limit of 2 quarters before renewal—a sum that we feel is appropriate for an MVP relative to the size of the treasury that UNI token holders are entrusted with allocating.

// **Purpose:**

// **The mission of the UGP is to provide valuable resources to help grow the swap ecosystem.** Through public discourse and inbound applications, the community will get first-hand exposure to identify and respond to the most pressing needs of the ecosystem, as well as the ability to support innovative projects expanding the capabilities of swap. By rewarding talent early with developer incentives, bounties, and infrastructure support, UGP acts as a catalyst for growth and helps to maintain swap as a nexus for DeFi on Ethereum.
...
// The UGP will be funded by the UNI treasury according to the[ release schedule](https://swap.org/blog/uni/) set out by the swap team, whereby 43% of the UNI treasury is released over a four-year timeline. In Year 1 this will total to 172,000,000 UNI (~$344M as of writing).
