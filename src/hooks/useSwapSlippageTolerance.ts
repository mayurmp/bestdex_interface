...
export default function useSwapSlippageTolerance(
  ...
  const defaultSlippageTolerance = useMemo(() => {
    ...
    try {
      const ethGasCost =
        ethGasPrice && typeof gasEstimate === 'number'
          ? JSBI.multiply(ethGasPrice, JSBI.BigInt(gasEstimate))
          : undefined
      const dollarGasCost =
        ether && ethGasCost && etherPrice
          ? etherPrice.quote(CurrencyAmount.fromRawAmount(ether, ethGasCost))
          : undefined
