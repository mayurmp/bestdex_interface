...
import { Currency, CurrencyAmount, Percent } from '@tech-alchemy/best-dex/sdk-core'
...
export default function AddLiquidity({
  ...
  const [approvalA, approveACallback, pendingApprovalStateA] = useApproveCallback(
    parsedAmounts[Field.CURRENCY_A],
    router?.address
  )
  const [approvalB, approveBCallback, pendingApprovalStateB] = useApproveCallback(
        ...
        <RowFlat style={{ marginTop: '20px' }}>
          {liquidityMinted ? (
            <>
              ...
              <Text fontSize="48px" fontWeight={500} lineHeight="42px" marginRight={10}>
                <MouseoverTooltip text={liquidityMinted?.toSignificant(6)}>
                  <TextEllipsis maxWidth="300px">{liquidityMinted?.toSignificant(6)}</TextEllipsis>
                </MouseoverTooltip>
              </Text>
            </>
          ) : (
            <Box>Insufficient input amount</Box>
          )}
        </RowFlat>
        <Row>
          <Text fontSize="24px">
  ...
  let isSupplyAllowed = true
  // Allowing supply only when output does not cross decimal limit
  try {
    const minCurrencyAllowed = parsedAmounts[Field.CURRENCY_B]?.toFixed(currencies?.CURRENCY_B?.decimals ?? 18)
    isSupplyAllowed = parseFloat(minCurrencyAllowed ?? '0') > 0
  } catch (e) {
    console.log(e)
  }
  return (
          ...
          <AutoColumn gap="20px">
            {noLiquidity ||
              (isCreate ? (
                ...
                <ColumnCenter>
                  <GreyCard>
                    <AutoColumn gap="10px">
                      <ThemedText.Link fontWeight={400} color={'swapSecondary'} style={{ fontSize: '14px' }}>
                        <Trans>
                          <b>
                            <Trans>Tip:</Trans>
                          </b>{' '}
            ...
            {addIsUnsupported ? (
              ...
              <AutoColumn gap={'md'}>
                  ...
                  isValid && (
                    <RowBetween>
                      {approvalA !== ApprovalState.APPROVED && (
                        ...
                        >
                          {approvalA === ApprovalState.PENDING || pendingApprovalStateA ? (
                            <Dots>
                      ...
                      {approvalB !== ApprovalState.APPROVED && (
                        ...
                        >
                          {approvalB === ApprovalState.PENDING || pendingApprovalStateB ? (
                            <Dots>
                ...
                liquidityMinted === undefined ? (
                  <SwapCallbackError error={'insufficient input amount'} />
                ) : null} */}
              </AutoColumn>
            )}
          </AutoColumn>
