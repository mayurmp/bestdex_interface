...
import { CurrencyAmount, Fraction, Percent, Price, Token } from '@tech-alchemy/best-dex/sdk-core'
import { FeeAmount, Pool, Position, priceToClosestTick, TickMath } from '@tech-alchemy/best-dex/v3-sdk'
...
function V2PairMigration({
  ...
  // approvals
  const [approval, approveManually, pendingApprovalState, setPendingApprovalState] = useApproveCallback(
    pairBalance,
    migrator?.address
  )
  ...
  }, [isNotUniswap, isArgentWallet, gatherPermitSignature, approveManually, setPendingApprovalState])
      ...
      .then((gasEstimate) => {
        return migrator
          ...
          .then((response: TransactionResponse) => {
            ReactGA.event({
              category: 'Migrate',
              action: `${isNotUniswap ? 'BestDex' : 'V2'}->V3`,
  ...
  return (
    <AutoColumn gap="20px">
      <ThemedText.Body my={9} style={{ fontWeight: 400 }}>
        <Trans>
          This tool will safely migrate your {isNotUniswap ? 'SWAPS' : 'V2'} liquidity to V3. The process is completely
        ...
        {chainId && migrator && (
          <ExternalLink href={getExplorerLink(chainId, migrator.address, ExplorerDataType.ADDRESS)}>
            <ThemedText.Blue display="inline">
              <Trans>BestDex migration contract↗</Trans>
            </ThemedText.Blue>
          </ExternalLink>
        )}
        .
        ...
        <AutoColumn gap="lg">
          ...
          {noLiquidity && (
            <BlueCard style={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
              ...
              >
                <Trans>
                  You are the first liquidity provider for this BestDex V3 pool. Your liquidity will migrate at the
                  current {isNotUniswap ? 'BestDex' : 'V2'} price.
                </Trans>
              </ThemedText.Body>
                ...
                <AutoColumn gap="8px" style={{ marginTop: '12px' }}>
                  <RowBetween>
                    <ThemedText.Body fontWeight={500} fontSize={14}>
                      <Trans>
                        {isNotUniswap ? 'BestDex' : 'V2'} {invertPrice ? currency1.symbol : currency0.symbol} Price:
                      </Trans>{' '}
                      <div style={{ verticalAlign: 'middle', display: 'inline-block', marginTop: '7px' }}>
                        <MouseoverTooltip
                          text={
                            invertPrice
                        ...
                        >
                          <TextEllipsis maxWidth={isMobile ? '150px' : '250px'}>
                            {invertPrice
                              ? `${v2SpotPrice?.invert()?.toSignificant(6)} ${currency0.symbol}`
                              : `${v2SpotPrice?.toSignificant(6)} ${currency1.symbol}`}
                          </TextEllipsis>
          ...
          {largePriceDifference ? (
            <YellowCard>
              <AutoColumn gap="8px">
                ...
                <RowBetween>
                  ...
                  <ThemedText.Black fontSize={14} color="inherit">
                    <MouseoverTooltip text={priceDifferenceFraction?.toSignificant(4) + '%'}>
                      <TextEllipsis maxWidth={isMobile ? '140px' : '250px'}>
              ...
              <ThemedText.Body fontSize={14} style={{ marginTop: 8, fontWeight: 400 }}>
                <Trans>
                  You should only deposit liquidity into BestDex V3 at a price you believe is correct. <br />
            ...
            {!isSuccessfullyMigrated && !isMigrationPending ? (
              <AutoColumn gap="12px" style={{ flex: '1' }}>
                <ButtonConfirmed
                  ...
                  onClick={approve}
                >
                  {approval === ApprovalState.PENDING || pendingApprovalState ? (
                    <Dots>
    ...
    <BodyWrapper style={{ padding: 24 }}>
      <AutoColumn gap="16px">
        <AutoRow style={{ alignItems: 'center', justifyContent: 'space-between' }} gap="8px">
          ...
          {/* hiding settings option from pool tab */}
