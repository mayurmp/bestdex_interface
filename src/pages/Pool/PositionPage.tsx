...
import { Currency, CurrencyAmount, Fraction, Percent, Price, Token } from '@tech-alchemy/best-dex/sdk-core'
import { NonfungiblePositionManager, Pool, Position } from '@tech-alchemy/best-dex/v3-sdk'
...
import { currencyReturnByChainId } from 'utils/currencyReturnByChainId'
...
const NFTImageContainer = styled.div`
  ...
  z-index: 2;
`
const ButtonGrayStyle = styled(ButtonGray)`
...
function NFT({ image, height: targetHeight }: { image: string; height: number }) {
  ...
  useEffect(() => {
    const getNFT = async () => {
      try {
        const nftRes = await fetch(image)
        const nft = await nftRes?.text()
        if (imageTagRef.current) {
          const updatedNft = nft.replaceAll('Courier New', 'Poppins')
          imageTagRef.current.innerHTML = updatedNft
          const gNode = imageTagRef.current?.querySelector('g')
          if (gNode && gNode?.childNodes?.[1]) {
            ;(gNode.childNodes[1] as HTMLElement).style.filter = ''
            ;(gNode.childNodes[1] as HTMLElement).setAttribute('fill', '#5a63ff')
            const topEllipse = gNode.querySelector('ellipse')
            if (topEllipse) {
              topEllipse.setAttribute('fill', '#3942db')
            }
            const textArray = imageTagRef.current?.querySelectorAll('text')
            if (textArray && textArray.length > 0) {
              textArray.forEach((text) => {
                text.style.fontFamily = 'SANS-SERIF'
              })
            }
            const svg = imageTagRef.current.querySelector('svg')
            if (svg && imageRef.current) {
              svg.setAttribute('height', '400')
              const blob = new Blob([svg.outerHTML as any], { type: 'image/svg+xml;charset=utf-8;' })
    ...
    getNFT()
  }, [image, imageRef, imageTagRef, canvasRef, targetHeight])
  return (
    <NFTGrid
      onMouseEnter={() => {
        setAnimate(true)
    ...
    >
      ...
      <NFTImageContainer hidden={true} ref={imageTagRef}></NFTImageContainer>
      <NFTImage
        ref={imageRef}
        // src={image}
        hidden={!animate}
                ...
                <AutoColumn gap="md" style={{ width: '100%' }}>
                  ...
                  {showCollectAsWeth && (
                    <AutoColumn gap="md">
                      <RowBetween>
                        <ThemedText.Main>
                          <Trans>Collect as {chainId ? currencyReturnByChainId(chainId) : 'WETH'}</Trans>
                        </ThemedText.Main>
                        <Toggle
                          id="receive-as-weth"
              ...
              <RowBetween>
                <LightCard padding="12px" width="100%">
                  <AutoColumn gap="8px" justify="center">
                    ...
                    <MouseoverTooltip text={formatTickPrice(priceLower, tickAtLimit, Bound.LOWER)}>
                      <ThemedText.MediumHeader textAlign="center">
                        <TextEllipsis maxWidth={isMobile ? '120px' : '300px'}>
                ...
                <LightCard padding="12px" width="100%">
                  <AutoColumn gap="8px" justify="center">
                    ...
                    <MouseoverTooltip text={formatTickPrice(priceUpper, tickAtLimit, Bound.UPPER)}>
                      <ThemedText.MediumHeader textAlign="center">
                        <TextEllipsis maxWidth={isMobile ? '120px' : '300px'}>
