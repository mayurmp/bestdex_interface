...
export default function RemoveLiquidity({
  ...
  const router = useV2RouterContract(V2_ROUTER_ADDRESS)
  ...
  const [approval, approveCallback, pendingApprovalState, setPendingApprovalState] = useApproveCallback(
    parsedAmounts[Field.LIQUIDITY],
    router?.address
  )
      ...
      <>
        <RowBetween>
          <Text color={theme.text2} fontWeight={500} fontSize={16}>
            <Trans>
              BD LP {currencyA?.symbol}/{currencyB?.symbol} Burned
            </Trans>
          </Text>
          <RowFixed>
  ...
  const getNativeTokenName = (chainId: any) => {
    switch (chainId) {
      case 1:
      case '1':
      case 5:
      ...
      <AppBody>
        ...
        <Wrapper>
          ...
          <AutoColumn gap="md">
            <GreyCard>
              <AutoColumn gap="10px">
                <ThemedText.Link fontWeight={400} color={'swapSecondary'}>
                  <Trans>
            ...
            {!showDetailed && (
              <>
                ...
                <LightCard>
                  <AutoColumn gap="10px">
                    <RowBetween>
                      <Text fontSize={isMobile ? 16 : 24} fontWeight={500}>
                        {formattedAmounts[Field.CURRENCY_A] || '-'}
                      </Text>
                      <RowFixed>
                        <CurrencyLogo currency={currencyA} style={{ marginRight: '12px' }} />
                        <Text fontSize={isMobile ? 16 : 24} fontWeight={500} id="remove-liquidity-tokena-symbol">
                          {currencyA?.symbol}
                        </Text>
                      </RowFixed>
                    </RowBetween>
                    <RowBetween>
                      ...
                      <RowFixed>
                        ...
                        <Text fontSize={isMobile ? 16 : 24} fontWeight={500} id="remove-liquidity-tokenb-symbol">
                          {currencyB?.symbol}
                        </Text>
                      </RowFixed>
                    </RowBetween>
                    {chainId && (oneCurrencyIsWETH || oneCurrencyIsETH) ? (
                      <RowBetween style={{ justifyContent: 'flex-end' }}>
                        {oneCurrencyIsETH ? (
                          ...
                          >
                            {`Receive ${chainId ? currencyReturnByChainId(chainId) : 'WETH'}`}
                          </StyledInternalLink>
                        ) : oneCurrencyIsWETH ? (
                          <StyledInternalLink
                            to={`/remove/v2/${
                              currencyA?.equals(WETH9_EXTENDED[chainId]) ? getNativeTokenName(chainId) : currencyIdA
                            }/${
                              currencyB?.equals(WETH9_EXTENDED[chainId]) ? getNativeTokenName(chainId) : currencyIdB
                            }`}
                          >
                            {`Receive ${chainId ? getNativeTokenName(chainId) : 'ETH'}`}
                          </StyledInternalLink>
                        ) : null}
                      </RowBetween>
                    ) : null}
            ...
            {pair && (
              <div style={{ padding: '10px 20px' }}>
                {!isMobile ? (
                  <div>
                    <RowBetween alignItems={'start'} textAlign={'end'}>
                      <Trans>Price:</Trans>
                      <div>
            ...
            <div style={{ position: 'relative' }}>
              {!account ? (
                <Button type="SECONDARY" size="XLARGE" center={true} fillWidth={true} onClick={toggleWalletModal}>
