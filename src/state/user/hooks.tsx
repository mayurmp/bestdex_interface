import { Percent, Token } from '@tech-alchemy/best-dex/sdk-core'
...
// returning false, as currently supporting only light theme
export function useIsDarkMode(): boolean {
  ...
  const wallet = query.get('client')
  const [isDarkMode] = useState(() => {
    // Check if the theme preference is stored in localStorage
    const storedTheme = sessionStorage.getItem('isDarkMode')
    // If it exists, return the stored value; otherwise, determine the theme based on the URL
    return storedTheme !== null ? JSON.parse(storedTheme) : wallet === 'bestwallet' ? false : true
  })

  // Update the theme preference in sessionStorage only if it doesn't exist
  useEffect(() => {
    const storedTheme = sessionStorage.getItem('isDarkMode')
