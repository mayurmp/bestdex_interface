import { TokenList } from '@tech-alchemy/best-dex/token-lists'
    ...
    if (!tokenListValidator) {
      tokenListValidator = new Promise<ValidateFunction>(async (resolve) => {
        const [ajv, schema] = await Promise.all([
          import('ajv'),
          import('@tech-alchemy/best-dex/token-lists/src/tokenlist.schema.json'),
        ])
...
export default async function getTokenList(
  ...
  for (let i = 0; i < urls.length; i++) {
    ...
    const isCoingecko = json.name === 'CoinGecko'
    if (isCoingecko) {
      // coingecko lists are not validatable
      const jsonNew = { ...json }
      jsonNew.version = {
        major: 0,
        minor: 2,
